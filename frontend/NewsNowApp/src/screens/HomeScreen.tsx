...
export default function HomeScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    const loadHomeNews = async () => {
      try {
        ...
        const mapped: NewsItem[] = Array.isArray(data)
          ? data.map((a: any) => {
              const catLevel = categoryLevels[a.category] ?? levelNum;
              return {
                id: String(a.id),
                title: a.title,
                cat: a.category,
                ...
                level: catLevel <= 1 ? '하' : catLevel === 2 ? '중' : '상',
  ...
  const recommended = useMemo(() => {
    const rec = homeNews.filter(n =>
      selectedCategories.some(c => n.cat === c || n.cat.includes(c) || c.includes(n.cat))
    );
    return rec.length >= 2 ? rec.slice(0, 4) : homeNews.slice(0, 4);
  }, [homeNews, selectedCategories]);
  ...
  const popular = useMemo(
    () => [...homeNews].sort((a, b) => b.views - a.views).slice(0, 5),
    [homeNews],
  );
